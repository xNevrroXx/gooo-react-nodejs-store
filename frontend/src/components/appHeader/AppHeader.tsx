...
function AppHeader() {
  ...
  return (
    ...
    <AppBar position="relative" sx={{mb: "1rem", zIndex: "100"}} >
      <Toolbar>
        <IconButton component={RouterLink} to="main" sx={{display: {xs: "none", sm: "block"}, justifyContent: "flex-start", mr: "1rem"}}>
...
const AuthenticationView = (user: RootState["authentication"]["user"], isOpen: boolean, onOpen: (event: React.MouseEvent<HTMLButtonElement>) => void, onClose: () => void, anchorEl: null | HTMLElement) => {
  ...
  if(user) {
    ...
    return (
        <Button component={RouterLink} to="/login" sx={{display: {xs: "none", sm: "flex"}, alignItems: "center", justifyContent: "center", flexDirection: "column", color: "inherit", textDecoration: "none"}}>
          <AccountCircleOutlined/>
          <Typography variant="body2" component="span">
            Войти
          </Typography>
